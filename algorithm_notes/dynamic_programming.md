...
### 核心思想
编辑距离(Levenshtein距离)是字符串相似性度量的经典问题，通过插入、删除、替换三种操作将一个字符串转换为另一个字符串的最少操作数。
...
### 状态定义与转移
**状态定义**：`dp[i][j]` = 将word1的前i个字符转换为word2的前j个字符的最少操作数

**状态转移**：
- 字符相同：`dp[i][j] = dp[i-1][j-1]`
- 字符不同：`dp[i][j] = 1 + min(删除, 插入, 替换)`

### 三种操作的理解
1. **删除**：`dp[i-1][j] + 1` - 删除word1[i-1]，然后解决剩余问题
2. **插入**：`dp[i][j-1] + 1` - 插入word2[j-1]，然后解决剩余问题  
3. **替换**：`dp[i-1][j-1] + 1` - 替换word1[i-1]为word2[j-1]，然后解决剩余问题

### 复杂度分析
- **时间复杂度**：O(mn) - 双重循环填表
- **空间复杂度**：O(mn) - 二维DP表；可优化到O(min(m,n))
    ...
    for(int j = 1; j <= n; j++) {
        ...
        for(int i = 1; i <= m; i++) {
            if(word1[i-1] == word2[j-1]) {
                ...
                curr[i] = 1 + min({pre[i], curr[i-1], pre[i-1]});
            }
        }
        swap(pre, curr);
    }
...
for(int i=0; i<n+1; i++) {
    dp[0][i] = 1;  // ❌ 应该是dp[0][j] = j
}
```

**原因分析**：
- `dp[i][0]`表示word1前i个字符转换为空字符串，需要删除i个字符，所以是i而不是1
- 循环变量命名混乱，第二个循环应该用j而不是i
...
**原因分析**：
- 变量名拼写错误，应该是i-1和j-1
- dp[i][j]对应的是word1[i-1]和word2[j-1]的比较

**正确写法**：
```cpp
if(word1[i-1] == word2[j-1]) {  // ✅ 正确的字符索引
```
...
#### 记录编辑操作序列
```cpp
// 不仅计算最小编辑距离，还能重构编辑操作序列
string reconstructOperations(string word1, string word2) {
    // 在DP过程中记录每步的操作选择
    // 最后通过回溯重构完整的操作序列
}
```
...
```cpp
// 不同操作有不同的代价
int weightedEditDistance(string word1, string word2, 
...
### 实际应用场景
1. **拼写检查**：计算单词与词典中词汇的相似度
2. **DNA序列比对**：生物信息学中的序列匹配
3. **版本控制**：Git中的文件差异比较
4. **自然语言处理**：文本相似度计算
5. **模糊搜索**：搜索引擎的容错匹配
...
| 问题 | 状态转移特点 | 应用场景 |
|------|-------------|----------|
| LCS | 匹配则延续，不匹配选最优 | 版本控制、相似度 |
| 编辑距离 | 三种操作取最小代价 | 拼写检查、序列比对 |
| 回文子序列 | 区间DP，两端匹配 | 字符串处理 |

### 算法精髓总结
编辑距离体现了动态规划在字符串处理中的威力：
1. **问题分解**：大问题分解为字符级别的子问题
2. **决策抽象**：三种编辑操作的统一数学建模
3. **最优子结构**：局部最优解构成全局最优解
4. **状态压缩**：二维问题可优化为一维空间

这是双序列DP的巅峰应用，掌握后可以轻松处理字符串相关的复杂动态规划问题。
